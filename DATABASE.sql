-- ============================================================
-- MDP DATA — قاعدة البيانات الكاملة (ملف واحد شامل)
-- ============================================================
-- شغّل الملف ده كامل مرة واحدة في Supabase SQL Editor.
-- آمن للتشغيل على مشروع جديد فاضي، أو لتحديث مشروع موجود بالفعل
-- (كل شيء مبني بـ "if not exists" ودروب/كرييت للـ policies، فمفيش
-- خطر إنه يمسح أي داتا موجودة).
--
-- منطق الصلاحيات النهائي: صلاحية Brand و صلاحية Partner لازم الاتنين
-- مع بعض (AND) عشان اليوزر يشوف الصف — تمامًا زي فلتر Excel.
-- ============================================================

-- ------------------------------------------------------------
-- 1) البروفايلات (بتتربط تلقائي بحساب تسجيل الدخول)
-- ------------------------------------------------------------
...
-- ------------------------------------------------------------
-- 2) القوائم الرئيسية (البراندات والبارتنرز)
-- ------------------------------------------------------------
...
-- ------------------------------------------------------------
-- 3) صلاحيات المستخدمين (Brand / Partner لكل يوزر)
-- ------------------------------------------------------------
...
-- ------------------------------------------------------------
-- 4) الداتا اليومية الرئيسية (بتتمسح وتتستبدل بالكامل كل رفعة)
-- ------------------------------------------------------------
...
-- ------------------------------------------------------------
-- 5) بيانات Achievement (MDP + AX / Miami Center)
-- ------------------------------------------------------------
...
-- لو الجدول كان موجود قبل كده من غير العمودين دول، الأسطر دي بتضيفهم بأمان
alter table public.archive_mdp_rows add column if not exists item_code text;
alter table public.archive_mdp_rows add column if not exists qty numeric not null default 0;

create index if not exists archive_mdp_brand_partner_idx on public.archive_mdp_rows (brand, partner);
create index if not exists archive_mdp_brand_idx on public.archive_mdp_rows (brand);
...
alter table public.archive_ax_rows add column if not exists qty numeric not null default 0;

create index if not exists archive_ax_brand_idx on public.archive_ax_rows (brand);
...
-- إزالة أي policies قديمة على كل الجداول (لو الملف ده اتشغل قبل كده)
do $$
declare tbl text; pol record;
begin
  foreach tbl in array array[
    'profiles','brands','partners','permissions','data_rows','data_columns',
    'archive_mdp_rows','archive_ax_rows'
  ]
  loop
...
-- ---- DATA_ROWS (فلتر Excel: Brand AND Partner) ----
...
-- ---- ARCHIVE_MDP_ROWS (نفس فلتر Excel: Brand AND Partner) ----
...
-- ---- ARCHIVE_AX_ROWS (Miami Center: يكفي صلاحية Brand بس) ----
...
-- ------------------------------------------------------------
-- الصلاحيات العامة (RLS برضو بتضبط اللي يشوفه كل يوزر فعليًا)
-- ------------------------------------------------------------
grant usage on schema public to authenticated;
grant select, insert, update, delete
on public.profiles, public.brands, public.partners, public.permissions,
   public.data_rows, public.data_columns,
   public.archive_mdp_rows, public.archive_ax_rows
to authenticated;
grant usage, select on all sequences in schema public to authenticated;

-- ------------------------------------------------------------
-- تأكيد إن كل الـ policies اتعملت صح (اختياري، بس للمراجعة)
-- ------------------------------------------------------------
...
-- ============================================================
-- بعد التشغيل: اعمل أول حساب (Authentication -> Users -> Add user)
-- وبعدين حوّله لأدمن بالسطر ده (بدّل الإيميل):
-- ============================================================
-- update public.profiles set is_admin = true where email = 'YOUR_EMAIL@mdp.local';
